...
class Transformation:
    ...
    def __init__(self, path_to_file):
        ...
        self.augmented_grey_scale = cv2.cvtColor(
            self.augmented_img,
            cv2.COLOR_BGR2GRAY
        )
    ...
    def gaussian_blur(self):
        self.gaussian_blur_img = cv2.GaussianBlur(
            self.augmented_grey_scale,
            (11, 11),
            sigmaX=0
        )
    ...
    def analyze_size(self):
        labeled_mask, _ = pcv.create_labels(self.binary_mask)
        self.shape_image = pcv.analyze.size(
            self.original,
    ...
    def pseudolandmarks(self):
        ...
        for i in range(len(top)):
            cv2.circle(
                self.pseudolandmarks_img,
                [int(top[i][0][0]), int(top[i][0][1])],
                radius,
                dark_blue,
                thickness
            )
            cv2.circle(
                self.pseudolandmarks_img,
                [int(bottom[i][0][0]), int(bottom[i][0][1])],
                radius,
                pink,
                thickness
            )
            cv2.circle(
                self.pseudolandmarks_img,
                [int(center_v[i][0][0]), int(center_v[i][0][1])],
                radius,
                orange,
                thickness
            )
...
def create_transformations(argv):
    if len(argv) == 2:
        ...
        for file in os.listdir(argv[1]):
            ...
            Image.fromarray(transformation.gaussian_blur_img).save(
                transformed_file_path_prefix + '-gaussian.JPG')
            Image.fromarray(transformation.mask).convert('RGB').save(
                transformed_file_path_prefix + '-mask.JPG')
            Image.fromarray(transformation.binary_mask).convert('RGB').save(
                transformed_file_path_prefix + '-binary-mask.JPG')
            Image.fromarray(transformation.shape_image).convert('RGB').save(
                transformed_file_path_prefix + '-analyze-object.JPG')
            Image.fromarray(transformation.pseudolandmarks_img).convert(
                'RGB').save(
                transformed_file_path_prefix + '-pseudolandmarks.JPG')
            Image.fromarray(transformation.canny_edges_img).convert(
                'RGB').save(
                transformed_file_path_prefix + '-canny-edges.JPG')
    else:
        sys.exit('usage: python3 Transformation.py <input_img> \
            or python3 Transformation.py <input_dir> <dest_dir>')
